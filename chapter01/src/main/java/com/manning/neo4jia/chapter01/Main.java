...
public class Main {
	...
	private static void runJdbcQuery(ApplicationContext context) {
		final FriendsOfFriendsFinder jdbcFriendsOfFriendsFinder = (FriendsOfFriendsFinder) context
				.getBean("jdbcFriendsOfFriendsFinder");
	...
	private static void generateGraphData() {
		...
		if (elementCnt != (COUNT + COUNT * FRIENDS_PER_USER)) {
			logger.info("Not enough users/friends ({}, required {}), deleting existing neo4j database manually.",
	...
	private static void generatedJdbcData(ApplicationContext context) {
		...
		// jdbcTemplate.execute("create table if not exists T_USER ( id int not
		// null, name varchar not null )");
		// jdbcTemplate.execute("create table if not exists T_USER_FRIEND ( id
		// int not null, user_1 int not null, user_2 int not null )");

		Integer userCount = jdbcTemplate.query("select count(id) as cnt from t_user",
		...
		Integer friendshipCount = jdbcTemplate.query("select count(id) as cnt from t_user_friend",
				new ResultSetExtractor<Integer>() {
					public Integer extractData(ResultSet rs) throws SQLException, DataAccessException {
						rs.next();
						return rs.getInt("cnt");
